...
// CREATE message (auth required) {postman tick}
// - Ignores client-sent name/profile_image and derives from User
// - Optional productId snapshot
...
export const addMessage = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      uploadImage = `${req.protocol}://${req.get("host")}/uploads/chat/${req.file.filename}`;
    }

    // Must have at least one payload
    if (!message && !uploadImage && !productId) {
